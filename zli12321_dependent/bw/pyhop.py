...
def pyhop(state,tasks,verbose=0):
    ...
    result = seek_plan(state,tasks,[],0, counter, verbose)
    if verbose>0: print('** result =',result,'\n')
    return (result, counter.count)

def seek_plan(state,tasks,plan,depth, count_obj, verbose=0):
    """
    ...
    if task1[0] in operators:
        ...
        if newstate:
            solution = seek_plan(newstate,tasks[1:],plan+[task1],depth+1,count_obj, verbose)
            if solution != False:
                return solution
    if task1[0] in methods:
        ...
        for method in relevant:
            ...
            if subtasks != False:
                solution = seek_plan(state,subtasks+tasks[1:],plan,depth+1, count_obj, verbose)
                if solution != False:
                    return solution
