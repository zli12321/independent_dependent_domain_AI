...
def pyhop(state, goals, verbose=0):
    ...
    result = seek_plan(state, goals, [], 0, counter, verbose)
    if verbose > 0:
        print('** result =', result, '\n')
    return (result, counter.count)


def seek_plan(state, goals, plan, depth, count_obj, verbose=0):
    """
    ...
    if getattr(state, goal1[0])[goal1[1]] == goal1[2]:  # Check whether goal1 is already satisfied
        if verbose > 2:
            ...
            print_state(state)
        solution = seek_plan(state, goals[1:], plan, depth + 1, count_obj, verbose)
    ...
    if goal1[0] in operators:
        ...
        for operator in relevant:  # Look for relevant operators that are applicable
            ...
            if newstate:
                ...
                solution = seek_plan(newstate, goals[1:], plan + [(operator.__name__,) + goal1[1:]], depth + 1, count_obj, verbose)
                if solution:
                    return solution
    if goal1[0] in methods:
        ...
        for method in relevant:  # Look for relevant methods that are applicable
            ...
            if subgoals:
                solution = seek_plan(state, subgoals + goals[1:], plan, depth + 1, count_obj, verbose)
                if solution:
                    return solution
    if verbose > 2:
